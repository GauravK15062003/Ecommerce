...
export const getProduct =
  ...
  async (dispatch) => {
    try {
      ...
      if (category) {
        link = `${process.env.REACT_APP_BACKEND_URL}/api/v1/products?keyword=${keyword}&page=${currentPage}&price[gte]=${price[0]}&price[lte]=${price[1]}&category=${category}&ratings[gte]=${ratings}`;
      }
...
export const deleteReviews = (reviewId, productId) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.delete(
      `${process.env.REACT_APP_BACKEND_URL}/api/v1/reviews?id=${reviewId}&productId=${productId}`
    );
